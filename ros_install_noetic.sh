...
#set -x
echo -n "Do you want to install ROS-Noetic? (y/N): "
read ros_install
...
if [ "$ros_install" '==' "y" ] || [ "$ros_install" '==' "Y" ]; then
    ...
    echo $PASSWORD | sudo add-apt-repository universe
    echo $PASSWORD | sudo add-apt-repository restricted
    echo $PASSWORD | sudo add-apt-repository multiverse
    ...
    echo -n "Skip install ROS Noetic."
fi
